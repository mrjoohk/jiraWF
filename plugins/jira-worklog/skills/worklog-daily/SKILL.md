...
- `schema_version`이 이 플러그인이 기대하는 값(**2**)과 다르면 **실행하지 말고 중단**한다. 잘못된 스키마로 계속 도는 것보다 멈추는 편이 복구 가능하다.
- **범위를 확정한다.** `state.json`의 `project_key`와 `epic_key`가 판단 기준이다. `jira_project_key_id.txt`는 사람이 읽기 위한 사본이며(주석·빈 줄 무시, 첫 유효 줄), 값이 `state.json`과 다르면 `state.json`을 따르되 **그 불일치를 사용자에게 보고한다.**
